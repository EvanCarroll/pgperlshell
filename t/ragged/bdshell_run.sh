...
bdshell -d dealermade -c '\COPY bdshell_test.ragged (a,b,c,d) FROM data/two_column_load_normal.csv WITH -PERL_static_column=$${"b": 2, "d": 4}$$;'

bdshell -d dealermade -c '\COPY bdshell_test.ragged (a,b,c,d) FROM data/ten_column_load_normal.csv WITH -PERL_ragged -PERL_static_column=$${"d": 4}$$;'

echo "This should error:";
bdshell -d dealermade -c '\COPY bdshell_test.ragged (a,b,c,d) FROM data/ten_column_load_normal.csv WITH -PERL_static_column=$${"d": 4}$$;'
